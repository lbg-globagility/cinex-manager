    ...
    public partial class frmMovieSched : Form
    {
                        ...
                        if (movieschedid == "")
                        {
                            ...
                            if (cbxintermision.Checked == true && txtintermision.Text.Trim() != "")
                            {
                                ...
                                while (currentdate.ToShortDateString() == movieenddate.ToShortDateString())
                                {
                                    ...
                                    {
                                        //DateTime dtnew = currentdate.Add(timeendspan);// remarked 3.16.2016
                                        DateTime dtnew = currentdate.Add(timestartspan);// added the last screening last full show to cross dates
        ...
        public bool addsked(DateTime currentdate, int cinemaid, int movieid, string movieschedid, string moviescheslistid, TimeSpan timestartspan, TimeSpan timeendspan)
        {
            ...
            try
            {
                ...
                if (timestartspan.Days < 1)
                {
                    ...
                    //DateTime curtimeend = new DateTime(currentdate.Year, currentdate.Month, currentdate.Day, (int)timeendspan.TotalHours, timeendspan.Minutes, 0);//RMB remarked 12.17.2014
                    int curday = currentdate.Day;
                    int inthour = (int)timeendspan.TotalHours;
                    //if ((timeendspan.TotalHours > 0) && (timeendspan.TotalHours < 4))//remarked 3.16.2016
                    if ((timeendspan.TotalDays >= 1) && (timeendspan.TotalHours >= 24))
                    {
                        curday += 1;
                        inthour -= 24;
                    }
                    
                    //DateTime curtimeend = new DateTime(currentdate.Year, currentdate.Month, curday, (int)timeendspan.TotalHours, timeendspan.Minutes, 0);//remarked 3.16.2016
                    DateTime curtimeend = new DateTime(currentdate.Year, currentdate.Month, curday, inthour, timeendspan.Minutes, 0);//remarked 5.31.2017

                    //DateTime curtimeend = new DateTime();
                    TimeSpan movielengthspan = dtduration.Value.TimeOfDay;//added 5.31.2017
                    TimeSpan timeendspan2 = timestartspan.Add(movielengthspan);//added 5.31.2017
                    if (timeendspan2.TotalDays >= 1)//added 5.31.2017
                        curtimeend = currentdate.Add(timeendspan2);//added 5.31.2017
        ...
        private void dgvMovies_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv.CurrentRow.Selected)
            {
                ...
                string hours = ((int)result.TotalHours).ToString();
                string minutes = String.Format("{0:00}", result.Minutes);
                //dtduration.Value = new DateTime(2014, 6, 2, (int)result.TotalHours, result.Minutes, 0);//remarked 5.31.2017
                dtduration.Value = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, (int)result.TotalHours, result.Minutes, 0);//added 5.31.2017
        ...
        private void removeSchedToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbCinema.Items.Count > 0)
                {
                    ...
                    if (val.ToString() != "System.Data.DataRowView" && val.ToString() != "" && val != null)
                    {
                        if (calsked.SelectedElementEnd.Date == calsked.SelectedElementStart.Date) //.SetViewRange(calview.SelectionStart, calview.SelectionEnd);
                        {
                            StringBuilder sb = new StringBuilder();
                            sb.Append("Are you sure you want to remove the schedule for ");
                            sb.Append(Environment.NewLine);
                            sb.AppendFormat("{0:MMM dd yyyy} at Cinema {1}, continue?", calsked.SelectedElementEnd.Date, val.ToString());
                            DialogResult ans = MessageBox.Show(sb.ToString(),
                    this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                            if (ans == System.Windows.Forms.DialogResult.Yes)
                            {
                                int intidx = cmbCinema.SelectedIndex;
                                ...
                                selectcalendardate(calsked.SelectedElementEnd.Date);
                                removedaysched();//added 12.04.2017 dec 04
        ...
        public void removedaysched()//added dec.04.2017
        {
            myconn = new MySqlConnection();
            myconn.ConnectionString = m_frmM._connection;
            int intid = -1;
            ...
            if (dgvResult.Rows.Count > 0)
            {
                for (int i = 0; i < dgvResult.Rows.Count; i++)
                {
                    //if (dgvResult.SelectedRows.Count == 1)
                    //{
                    intid = Convert.ToInt32(dgvResult.Rows[i].Cells[0].Value.ToString());
                    movieid = Convert.ToInt32(dgvResult.Rows[i].Cells[5].Value.ToString());
                    //}
