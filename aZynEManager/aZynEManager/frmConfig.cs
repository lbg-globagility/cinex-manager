    ...
    public partial class frmConfig : Form
    {
        ...
        private void btnapply_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //RMB 04.06.2015 for accreditation start date START
                sqry = new StringBuilder();
                sqry.Append(String.Format("update config_table set system_value = '{0}' ", m_clscom.m_clscon.CinemaAccreditationNo));
                sqry.Append(String.Format("where system_code = '{0}' ", "018"));
                if (myconn.State == ConnectionState.Closed)
                    myconn.Open();
                cmd = new MySqlCommand(sqry.ToString(), myconn);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
                //RMB 04.06.2015 for accreditation start date END
