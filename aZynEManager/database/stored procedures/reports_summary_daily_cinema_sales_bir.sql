...
DROP PROCEDURE IF EXISTS `reports_summary_daily_cinema_sales_bir`;

CREATE PROCEDURE `reports_summary_daily_cinema_sales_bir`(_start_date DATETIME)
BEGIN
...
SELECT `name` INTO _report_name FROM report WHERE id = 16;
SET _dt = NOW();
...
SELECT IFNULL(SUM(a.price), 0)
 FROM movies_schedule_list_reserved_seat a, ticket b
WHERE a.ticket_id = b.id AND a.status = 1 AND DATE(ticket_datetime) <= _start_date;

SELECT IFNULL(SUM(a.price), 0)
 FROM movies_schedule_list_reserved_seat a, ticket b
WHERE a.ticket_id = b.id AND a.status = 1 AND DATE(ticket_datetime) <= DATE_SUB( _start_date, INTERVAL 1 DAY);
