...
{
    public class RP16 : IPreviewReport
    {
        public DateTime? StartDate { get; set; }

        public RP16(DateTime? dtStartDate)
        {
        ...
        public void PreviewReport()
        {
            ...
            double dblGrandTotalYesterdaySales = 0.0;

            using (MySqlConnection connection = new MySqlConnection(CommonLibrary.CommonUtility.ConnectionString))
            {
                using (MySqlCommand command = new MySqlCommand("reports_summary_daily_cinema_sales_bir", connection))
                {
                    ...
                    if (reader.NextResult())
                    {
                        if (reader.Read())
                        {
                            dblGrandTotalSales = reader.GetDouble(0);
                        }
                    }
                    ...
                    if (reader.NextResult())
                    {
                        if (reader.Read())
                        {
                            dblGrandTotalYesterdaySales = reader.GetDouble(0);
                        }
                    }
            ...
            newFile.Worksheets[0].Cells[intRowCount, 0].Value = "GRAND TOTAL TODAY:";
            newFile.Worksheets[0].Cells[intRowCount, 0].Style.Font.Weight = ExcelFont.BoldWeight;
            ...
            newFile.Worksheets[0].Cells[intRowCount, 0].Value = "GRAND TOTAL YESTERDAY:";
            newFile.Worksheets[0].Cells[intRowCount, 0].Style.Font.Weight = ExcelFont.BoldWeight;
            newFile.Worksheets[0].Cells[intRowCount, 5].Value = string.Format("{0:#,##0.00}", dblGrandTotalYesterdaySales);
            newFile.Worksheets[0].Cells[intRowCount, 5].Style.HorizontalAlignment = HorizontalAlignmentStyle.Right;
            newFile.Worksheets[0].Cells[intRowCount, 5].Style.Font.Weight = ExcelFont.BoldWeight;
