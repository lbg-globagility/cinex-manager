...
{
    public class RP10 : IPreviewReport
    {
        public DateTime? StartDate { get; set; }
        public DateTime? EndDate { get; set; }

        public RP10(DateTime? dtStartDate, DateTime? dtEndDate)
        {
        ...
        public void PreviewReport()
        {
            ...
            {
                using (MySqlCommand command = new MySqlCommand("reports_cinema_sales_ticket_type_sold_per_movie_title", connection))
                {
                    ...
                    if (reader.NextResult())
                    {
                        while (reader.Read())
                        {
                            ...
                            newFile.Worksheets[0].Cells[intRowCount, 2].Value = string.Format("{0:#,##0.00}", reader.GetDouble(4));
                            newFile.Worksheets[0].Cells[intRowCount, 2].Style.HorizontalAlignment = HorizontalAlignmentStyle.Right;

                            intQuantity = reader.GetInt32(5);
